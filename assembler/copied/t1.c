...
Initialize() {
    int i = 0, j = 1;
    FILE *regi, *inst;
    regi = fopen("register_list.txt", "r");
    inst = fopen("instruction_list.txt", "r");
    while (!feof(regi)) {
        fscanf(regi, "%s%s\n", Reg[i].reg_name, Reg[i].reg_num);
        i++;
    } // 레지스터 테이블을 작성한다
    while (!feof(inst)) {
        fscanf(inst, "%6s%2s%2s%4s%3s%2s%9s\n", Instr[j].instruct, Instr[j].dest, Instr[j].sour, Instr[j].word_type,
               Instr[j].ins_code, Instr[j].ins_len, Instr[j].mod_reg);
        j++;
    } // 명령어 테이블을 작성한다.
    MaxI = j - 1;
    fclose(regi);
...
int
Analyze(char* operand) {
    int i = 0;
    char* regist[] = {"AX", "BX", "CX", "DX", "AL", "BL", "CL",
                      "DL", "AH", "BH", "CH", "DH", 0x00}; // 레지스터의 이름을 저장
...
#define MAX_INS 1 // 명령어의 초대 개수를 지정

int
Add_Chk(char* sen) // 오퍼랜드의 어드레스 모드를 판정
{
    register int k = MaxI;
    ...
    strcpy(Instr[0].instruct, op[0]); // OP코드를 저장
    switch (Analyze(op[1])) {
        case 0: strcpy(Instr[0].dest, "i"); break;
        case 1:
        ...
        case 2:
            ...
            break;
        case 3: strcpy(Instr[0].dest, "m"); break;
    }
    switch (Analyze(op[2])) {
        case 0: strcpy(Instr[0].sour, "i"); break;
        case 1:
